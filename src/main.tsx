...
const AdminDashboard = ({ user }: { user: FirebaseUser }) => {
  ...
  const [config, setConfig] = useState<AppConfig>({ proPrice: 10, manualPaymentInfo: 'Pay via PayPal: admin@example.com' });
...
const ChatView = ({ user, profile }: { user: FirebaseUser, profile: UserProfile }) => {
  ...
  // Cleanup typing status on unmount or group change
  useEffect(() => {
    return () => {
      if (selectedGroup) {
        updateTypingStatus(false);
  ...
  return (
    <div className="flex flex-col h-full bg-[#0b141a]">
      ...
      {isSearching && searchQuery === '' && recentSearches.length > 0 && (
        <div className="bg-[#111b21] p-4 border-b border-zinc-800">
          <h4 className="text-[10px] text-zinc-500 uppercase font-bold mb-2">Recent Searches</h4>
          <div className="flex flex-wrap gap-2">
            {recentSearches.map((s) => (
              <button 
                key={s.id}
                onClick={() => setSearchQuery(s.query)}
                className="px-3 py-1 bg-[#202c33] rounded-full text-xs text-zinc-300 hover:bg-[#2a3942]"
              >
                {s.query}
              </button>
            ))}
    ...
    <div className="h-full bg-[var(--bg-app)] flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide">
        ...
        <div className="p-4 space-y-4">
          ...
          {/* Visually Distinct Ad Component */}
...
const MatchView = ({ user, profile }: { user: FirebaseUser, profile: UserProfile }) => {
  ...
  useEffect(() => {
    ...
    // Matching logic: Same location, opposite gender
    const q = query(
      collection(db, 'users'), 
            ...
            <div className="space-y-2">
              {profile.role === 'admin' && (
                <button 
                  ...
                  className="w-full p-4 bg-emerald-500/10 text-emerald-500 rounded-2xl border border-emerald-500/20 flex items-center justify-between hover:bg-emerald-500/20 transition-colors shadow-sm"
                >
                  <div className="flex items-center gap-3">
